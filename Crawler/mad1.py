...
def crawl_page(keyword, start_date, end_date, sort, page):
    ...
    try:
        ...
        # user-agent 정보 기입이 헷갈리시면 오른쪽 링크를 참고해주세요. https://herjh0405.tistory.com/11
        # 아래줄에 입력되는 user-agent에 기입 되는 내용은 오른쪽 링크에서 확인하시면 됩니다. http://www.useragentstring.com/
...
easy_split = "2020.10.02 ~ 2020.10.06"

start_date = easy_split[:10]
end_date = easy_split[13:]

# 현재 진행 중인 기간 2020.10.02 ~ 2020.10.06 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
