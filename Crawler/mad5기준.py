...
def crawl_page(keyword, start_date, end_date, sort, page):
    ...
    try:
        ...
        # driver.wait_for_request(url, timeout=5)
        # driver.close()
        ...
        # if page_soup.find('strong', class_="title") is not None:
        #     a = 0/0
        # print(time.time() - aaa , "파싱 속도 측정")
        ...
        for link in links:
            ...
            # 하기 세 줄을 주석 처리 할 것
            print()
            tmp1 = round(time.time() - aa,3) 
            print(tmp1, "초 소요 (한 페이지 크롤링 소요시간)")
